...
```bash
recordings$ sam logs -n HelloEfsFunction --stack-name recordings --tail
```
...
```bash
recordings$ pip install -r tests/requirements.txt --user
# unit test
recordings$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
recordings$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name recordings
```
